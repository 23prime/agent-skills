...
2. その後、メインスレッドで直接 `refining-github-issue` を呼び出す。このスキル自身のステップ2（リファイン済みなら再質問しない挙動）により、これは安価な再チェックになる：更新済みの Issue を新鮮な目で読み直し、ステップ1の中継で失われたかもしれない点や、2回目に読んで初めて気づく不明点があればユーザーに直接確認する。
...
| ステップ | スキル |
| -------- | ------ |
| 1 | `issue-refiner`（エージェント）→ `refining-github-issue` |
