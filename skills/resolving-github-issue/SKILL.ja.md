...
1. `issue-refiner` エージェントを（フォアグラウンド、`run_in_background: false`）Issue 番号または URL で起動し、このワークフローの他のステップより高い推論努力でインタビューを実行させる。サブエージェントの起動はコンテキストの壁であり、コールドスタートで最終レポートしかメインスレッドに戻らないため、インタビューの各ターンを中継する：エージェントの質問はそのままユーザーに転送し、ユーザーの回答は `SendMessage` で同じエージェントに送り返す。これをエージェントが Issue の更新を終えるまで繰り返す。
2. `issue-reviewer` エージェントを（フォアグラウンド）Issue 番号または URL のみで起動する — ステップ1の議論内容は要約して渡さない。ステップ1の各ターンを自分で中継した以上、自分自身で読み直しても本当の意味でのフレッシュな目にはならない。事前情報ゼロのコールドなサブエージェントであって初めてそれが成立する。指摘があればユーザーに直接確認して Issue を更新し、なければそのまま進む。
