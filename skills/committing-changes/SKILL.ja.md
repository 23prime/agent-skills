...
- フォーマットは `<type>: <簡潔な要約>` とする。
- 英語で記述する。
- *何をしたか* だけでなく *なぜそうしたか* を要約する。
- 理由やコンテキストが自明でない場合のみ、任意の本文（3行目以降）を追加する。

### 5. ユーザーへの確認：デフォルトブランチへ直接コミットする場合のみ

現在のブランチとリポジトリのデフォルトブランチを確認する：
...
- **現在のブランチがトピック/フィーチャーブランチ**（デフォルトブランチではない）：
  確認をスキップし、そのままステップ 6 へ進む。
- **現在のブランチがデフォルトブランチそのもの**（main/master へ直接コミットする場合）：
  提案するコミットメッセージとステージ済みファイルの一覧をユーザーに提示し、
  承認を得てから次に進む。変更の要望があれば修正し、再度確認する。
